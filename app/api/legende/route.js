...
export async function POST(req) {
  ...
  if (!description || !platform || !langue) return Response.json({ error: 'Paramètres manquants.' }, { status: 400 });
  if (description.length > 500) return Response.json({ error: 'Description trop longue.' }, { status: 400 });

  const prompt = 'Tu es un expert en creation de contenu sur ' + platform + '. Genere une legende professionnelle et des hashtags viraux pour une video sur : ' + description + '. Langue : ' + langue + '. Format strict sans asterisques:\nLEGENDE:\n[legende ici]\nHASHTAGS:\n[hashtags ici]';
